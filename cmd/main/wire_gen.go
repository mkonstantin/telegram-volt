...
func InitializeApplication(secret string, logger *zap.Logger) (telegram.TelegramBot, func(), error) {
	...
	seatList := handler.NewSeatListHandle(bookSeatRepository, freeSeatMenu, logger)
	ownSeatMenu := handler.NewOwnSeatMenuHandle(userService, messageFormer, logger)
	handlerFreeSeatMenu := handler.NewFreeSeatMenuHandle(userService, messageFormer, logger)
	routerRouter := router.NewRouter(start, officeList, handlerOfficeMenu, seatList, ownSeatMenu, handlerFreeSeatMenu, logger)
